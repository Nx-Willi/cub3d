...
/*   Updated: 2022/08/27 16:27:43 by xle-baux         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	get_type_id(t_info *info, char **line)
{
	...
	if (ft_strncmp(line[0], "NO", 3) == 0)
		info->textures[T_NO] = ft_strdup(line[1]);
	else if (ft_strncmp(line[0], "SO", 3) == 0)
		info->textures[T_SO] = ft_strdup(line[1]);
	else if (ft_strncmp(line[0], "WE", 3) == 0)
		info->textures[T_WE] = ft_strdup(line[1]);
	else if (ft_strncmp(line[0], "EA", 3) == 0)
		info->textures[T_EA] = ft_strdup(line[1]);
