...
/*   Updated: 2022/08/06 14:06:06 by xle-baux         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int			is_duplicate_info(t_check_map *check_map_struct, char *line);
...
int	check_map(char **map)
{
	...
	while (map[i])
	{
		while (map[i] && is_empty_line(map[i]) == TRUE)
			i++;
		if (is_info(&check_map_struct, map[i]) == DUPLICATE)
			return (FALSE);
		else if (is_info(&check_map_struct, map[i]) == FALSE && is_map(map[i]) == FALSE)
		{
			printf("Error\nCould't get info from line %i: %s", i, map[i]);
			return (FALSE);
		}
		i++;
	}
